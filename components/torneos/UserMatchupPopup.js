...
const UserMatchupPopup = ({ onClose, leagueId, discordId }) => {
    ...
    useEffect(() => {
        const fetchMatches = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`https://api.duellinks.pro/leagues/${leagueId}/rounds/current/matches?discordId=${discordId}`);
